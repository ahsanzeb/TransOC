...
!******************************************************
	...
	! If PermSym; rates for 27=28; 29=30; 33=34; avoid repeat calc
		...
		! maz: May 2019: interested in dhops rate only
		! ih1, is loops restricted to dhops_right for a single site.
		do ih1=1,1!ih2
			ih = ihs(ih1)
			rate(ih)%rcs(:,:) = 0.0d0
			do is=1,1 !ways(ih)%ns,1
				do ic=1,nc
...
!******************************************************
	...
	iter = 1;
	do iii=1,nsec
		if(eig(itl)%esec(iii)-Einit > 1.0d-8) then
			exit
		endif
		iter = iii
	enddo
	...
	if(simplepf .or. ih==25) then 
     ...
     .  tvr * sum(PenaltyArray(de,nsec) * qt(ia)%cs(icl,isa)%amp2)
		...
		rate(ih)%rcs(ic,is) =   ! maz: 24May - dhops
